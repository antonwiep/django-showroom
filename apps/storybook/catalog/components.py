...
COMPONENTS_ROOT = Path(settings.BASE_DIR) / "design-system" / "cotton" / "ds"
FOUNDATIONS_ROOT = Path(settings.BASE_DIR) / "apps" / "storybook" / "foundations"
...
def _is_allowed_component(entry: dict):
    allowlist = getattr(settings, "STORYBOOK_COMPONENT_ALLOWLIST", None)
    if not allowlist:
        return True
