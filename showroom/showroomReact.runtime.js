...
import { showroomReactRegistry } from "./showroomReact.registry.generated";
...
function getStoryRenderer(componentSlug, storySlug) {
  const componentStories = showroomReactRegistry?.[componentSlug];
...
function sync({ componentSlug, storySlug, args = {}, onArgsChange } = {}) {
  ...
  for (const target of targets) {
    ...
    const portalContainer =
      target.closest("[data-showroom-zoom-root]") ??
      target.closest("[data-showroom-preview-pane]") ??
      target.closest("[data-showroom-preview-root]") ??
